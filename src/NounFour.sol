...
import {
    NounsToken,
    INounsSeeder
} from "nouns-monorepo/packages/nouns-contracts/contracts/NounsToken.sol";
import {
    NounsDescriptorV2,
    ISVGRenderer
} from "nouns-monorepo/packages/nouns-contracts/contracts/NounsDescriptorV2.sol";
import {NounsDAOLogicV1} from
    "nouns-monorepo/packages/nouns-contracts/contracts/governance/NounsDAOLogicV1.sol";
import {NounsDAOStorageV1} from
    "nouns-monorepo/packages/nouns-contracts/contracts/governance/NounsDAOInterfaces.sol";
...
contract NounFour is ERC721("NounFour", "N4"), ERC721TokenReceiver {
    ...
    NounsDAOLogicV1 public immutable nounsDAO;
    // tokenId => partInfo
    ...
    constructor(
        NounsToken _nouns,
        NounsDescriptorV2 _descriptors,
        NounsDAOLogicV1 _nounsDAO
    ) {
        nouns = _nouns;
    ...
    function castVote(uint256 tokenId, uint256 proposalId, uint8 support)
        external
    {
        ...
        returns (string memory)
    {
        ...
        string memory name = string.concat(
            partType, " of Noun ", Strings.toString(partInfo[id].nounId)
        );
        ...
        internal
    {
        ...
        partInfo[tokenId] = PartInfo({part: part, nounId: uint248(nounId)});
    }
    ...
    function partSVG(uint256 id)
        public
        view
        returns (string memory partType, string memory svg)
    {
        PartInfo storage info = partInfo[id];
        (
            uint48 background,
        ...
        ISVGRenderer.Part memory svgPart =
            ISVGRenderer.Part({image: partBytes, palette: _getPalette(partBytes)});

        string memory renderedSVGPart =
            descriptors.renderer().generateSVGPart(svgPart);

        svg = string.concat(
            _SVG_START_TAG,
            '<rect width="100%" height="100%" fill="#',
            descriptors.art().backgrounds(background),
            '" />',
            renderedSVGPart,
            _SVG_END_TAG
        );
